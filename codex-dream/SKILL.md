---
name: codex-dream
description: "Use when the user asks Codex to review recent work history, Codex sessions, task summaries, Codex Memories, rollout summaries, Chronicle activity, existing skills, custom agents, or automations to identify repeated manual workflows worth packaging; produce a shortlist, then create or extend only high-confidence missing workflow assets such as skills, custom subagents, automations, or playbooks."
---
...
- Prefer verifiable local evidence. When evidence is uncertain, say so and avoid creating assets from weak signals.
- Use Chronicle only for discovery. Confirm important details in the relevant source system when possible.
- Before creating anything, inspect existing skills, custom agents, and automations so the result extends or reuses what is already adequate.
- When creating a Skill, follow the local skill-creator convention: create `SKILL.md` and recommended `agents/openai.yaml`; keep `SKILL.md` frontmatter to `name` and `description` only.
- When creating an automation, first discover and use the available automation tool. Do not emit raw automation directives by hand.
- When creating or extending a custom subagent, first inspect `/Users/jupiter.k/.codex/agents` and prefer extending or reusing an existing agent when it already covers the role.
- Do not create speculative, overlapping, overly broad, sensitive, or poorly evidenced assets.
- Do not revive or depend on deleted predecessor workflow-packaging assets unless the user explicitly asks for that exact asset.
